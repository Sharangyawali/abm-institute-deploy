...
const page = ({ params }) => {
    ...
    const getDetail=async()=>{
        let result=await fetch(`/api/admin/visitors/${params.id}`,{
            method:'get'
        })
  ...
  <div className="w-[100%] tablet:w-[80%] flex flex-col laptop:flex-row justify-between items-center">
    ...
    <div className="w-[100%] laptop:w-[59%] flex flex-col h-[600px] shadow-xl rounded-md justify-evenly px-[10px] ">
      ...
      <Link href={`/admin/visitor/transaction/${detail.id}`} className="w-[150px] my-[15px] h-[30px] flex items-center justify-center bg-[#a25dda] font-semibold text-white float-end rounded-lg cursor-pointer">View Transactions</Link>
      </div>
      <div className="w-[100%] text-[18px] font-semibold text-[#3f3f3f]">
        Visits
        <hr></hr>
      ...
      <div className="flex w-[100%] justify-start">
      <Link href={`/admin/visitor/visits/${detail.id}`} className="w-[150px] my-[15px] h-[30px] flex items-center justify-center bg-[#a25dda] font-semibold text-white float-end rounded-lg cursor-pointer">View Visits</Link>
      </div>
    </div>
  </div>
    }
