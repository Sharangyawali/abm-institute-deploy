...
const page = () => {
  ...
  const getDetail = async () => {
    ...
    if (result.success === false) {
      ...
      setStreetAddress(result.detail.accountant.streetAddress ?? "");
      setCity(result.detail.accountant.city ?? "");
      setState(result.detail.accountant.state ?? "");
      setGender(result.detail.accountant.gender ?? "Male");
    }
  };
        ...
        <div className="w-[100%] tablet:w-[80%] flex flex-col laptop:flex-row justify-between items-center">
          <div className="w-[100%] laptop:w-[39%] flex flex-col justify-center items-center gap-[20px] h-[200px] laptop:h-[600px] shadow-xl rounded-md bg-gradient-to-r from-[#f06273] to-[#f0936a] text-white font-bold text-[18px]">
            <div className="h-[80px] w-[80px] relative rounded-full bg-white">
              <img
                src={`${selectedImage?selectedImage:gender==='Male'?'/profile-demo.jpg':'/female.png'}`}
